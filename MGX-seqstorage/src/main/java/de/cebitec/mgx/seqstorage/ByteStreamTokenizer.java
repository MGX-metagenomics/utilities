...
public class ByteStreamTokenizer implements Iterator<byte[]> {
    ...
    public ByteStreamTokenizer(String fname, boolean gzipCompressed, byte separatorChar, int skipBytes) throws IOException, SeqStoreException {
        ...
        if (gzipCompressed) {
            in = new GZIPInputStream(new FileInputStream(fname), 49152);
        } else {
    ...
    public boolean hasNext() {
        ...
        while (!atEOF && (breakpos = getSeparatorPos(startpos, endpos)) == -1) {
            ...
            // do not double the buffer size unless we have reached 90%
            // buffer usage
            if (endpos > bufferRefillPosition) {
                enlargeBuffer();
            }
    ...
    private boolean fillBuffer() {
        ...
        try {
            if ((bytesRead = in.read(buffer, endpos, buffer.length - endpos)) != -1) {
                ...
                endpos += bytesRead;
//                System.err.println("filled buffer with additional "+bytesRead+" up to " + endpos + ", fpos " + filePos);
            } else {
                atEOF = true;
                return true;
            }
...
//        System.err.println("buffer doubling to " + buffer.length * 2 + ", startpos " + startpos + ", endpos " + endpos);
